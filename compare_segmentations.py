...
from evaluation_comparison.pairwise_bundles import PairwiseMeasuresStreams
from evaluation_comparison.pairwise_bundles import CharBundle
...
MEASURES_STREAMS = ('ref volume', 'seg volume', 'ref numb', 'seg numb',
                    'dist_com', 'start_comp', 'end_comp', 'prob_match',
                    'length_match', 'summary_match')
...
def run_compare(param):
    ...
    with open(os.path.join(dir_init, out_name), 'w+') as out_stream:
        ...
        elif param.analysis == 'streams':
            m_headers = PairwiseMeasuresStreams(0, 9,
                                                measures=MEASURES_STREAMS).header_str()
        ...
        if param.analysis == 'streams':
            for i, pair_ in enumerate(pair_list):
                print("Processing %d out of %d %s vs %s" % (i+1,
                                                            len(pair_list), pair_[0],
                                                          pair_[1]))
                seg_name = pair_[0]
                ref_name = pair_[1]
                seg_streams = nib.streamlines.load(seg_name)
                ref_streams = nib.streamlines.load(ref_name)
                shape_seg = seg_streams._header['dimensions']
                shape_ref = ref_streams._header['dimensions']
                seg_streamobj = CharBundle(seg_streams, seg_streams.affine,
                                           shape_seg)
                ref_streamobj = CharBundle(ref_streams, ref_streams.affine,
                                           shape_ref)
                pe = PairwiseMeasuresStreams(seg_streamobj, ref_streamobj,
                                                  measures=MEASURES_STREAMS)
                fixed_fields = "{}, {},".format(ref_name, seg_name)
